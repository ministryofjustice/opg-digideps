...
PGPASSWORD=$API_DATABASE_PASSWORD psql -P pager=off -U $API_DATABASE_USERNAME -h $API_DATABASE_HOSTNAME -p $API_DATABASE_PORT -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$API_DATABASE_NAME' AND pid <> pg_backend_pid();"
