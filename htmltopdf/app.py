...
def application(request):
    ...
    with tempfile.NamedTemporaryFile(suffix=".html") as source_file:
        ...
        if not bucket_name:
            logger.error("BUCKET environment variable is not set; skipping S3 upload.")
        else:
            ...
            try:
                s3.upload_file(
                    ...
                    ExtraArgs={
                        ...
                        "ServerSideEncryption": "AES256",  # <‑‑ AES256 at rest
                    },
                )
                logger.info(
                    "Uploaded HTML to s3://%s/%s with AES256 SSE", bucket_name, s3_key
                )
