resource "aws_ecs_task_definition" "squid" {
  family                   = "squid-${local.environment}"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = var.account.cpu_low
  memory                   = var.account.memory_low
  container_definitions    = "[${local.squid}]"
  task_role_arn            = aws_iam_role.front.arn
  execution_role_arn       = aws_iam_role.execution_role.arn
  tags                     = var.default_tags
}

resource "aws_ecs_service" "squid" {
  name                    = aws_ecs_task_definition.squid.family
  cluster                 = aws_ecs_cluster.main.id
  task_definition         = aws_ecs_task_definition.squid.arn
  desired_count           = var.account.task_count
  platform_version        = "1.4.0"
  enable_ecs_managed_tags = true
  propagate_tags          = "SERVICE"
  wait_for_steady_state   = true
  tags                    = var.default_tags

  network_configuration {
    security_groups  = [module.squid_service_security_group.id]
    subnets          = data.aws_subnet.private[*].id
    assign_public_ip = false
  }

  service_connect_configuration {
    enabled   = true
    namespace = aws_service_discovery_http_namespace.cloudmap_namespace.arn
    service {
      discovery_name = "squid"
      port_name      = "squid-port"
      client_alias {
        dns_name = "squid"
        port     = 3128
      }
    }
  }

  capacity_provider_strategy {
    capacity_provider = local.capacity_provider
    weight            = 1
  }

  deployment_controller {
    type = "ECS"
  }

  deployment_circuit_breaker {
    enable   = false
    rollback = false
  }

  depends_on = [
    aws_ecs_service.api,
    aws_ecs_service.htmltopdf,
    aws_ecs_service.scan
  ]
}

locals {
  squid = jsonencode(
    {
      cpu         = 0,
      essential   = true,
      image       = local.images.squid-proxy,
      mountPoints = [],
      name        = "squid",
      portMappings = [{
        name          = "squid-port",
        containerPort = 3128,
        hostPort      = 3128,
        protocol      = "tcp"
      }],
      volumesFrom = [],
      logConfiguration = {
        logDriver = "awslogs",
        options = {
          awslogs-group         = aws_cloudwatch_log_group.opg_digi_deps.name,
          awslogs-region        = "eu-west-1",
          awslogs-stream-prefix = "${aws_iam_role.front.name}.squid"
        }
      },
      environment = []
    }
  )
}
