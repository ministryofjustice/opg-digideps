locals {
  api_rds_sg_rules = {
    ...
    ssm_ec2_data_access = {
      ...
      type        = "ingress"
      target_type = "security_group_id"
      target      = data.aws_security_group.ssm_data_access.id
    }
    db_access_tasks = {
      port        = 5432
      type        = "ingress"
    ...
    custom_sql_lambda = {
      ...
      protocol    = "tcp"
      target_type = "security_group_id"
      target      = data.aws_security_group.lambda_custom_sql_tool.id
    }
  }
}
...
resource "aws_security_group_rule" "postgres_ssm_egress_data_access" {
  ...
  security_group_id        = data.aws_security_group.ssm_data_access.id
}
