...
# Deputyships CSV Processing Check
resource "aws_cloudwatch_event_rule" "deputyships_csv_processing_check" {
...
resource "aws_cloudwatch_event_target" "deputyships_csv_processing_check" {
  target_id = "check-deputyships-csv-processing-${terraform.workspace}"
  arn       = data.aws_lambda_function.monitor_notify_lambda.arn
  rule      = aws_cloudwatch_event_rule.deputyships_csv_processing_check.name
  input = jsonencode(
    {
      scheduled-event-detail = {
        job-name                   = "deputyships_csv_processing_check"
        log-group                  = terraform.workspace,
        log-entries                = ["deputyships_csv_processing"],
        search-timespan            = "24 hours",
        bank-holidays              = "true",
        channel-identifier-absent  = "team",
        channel-identifier-success = "scheduled-jobs",
