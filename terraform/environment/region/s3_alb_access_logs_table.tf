resource "aws_glue_catalog_table" "alb_logs" {
  ...
  storage_descriptor {
    ...
    location                  = "s3://alb-logs.${data.aws_region.current.name}.${local.s3_alb_log_account_name}.digideps.opg.justice.gov.uk/${local.environment}/AWSLogs/${var.account.environment.account_id}/elasticloadbalancing/${data.aws_region.current.name}"
    number_of_buckets         = -1
