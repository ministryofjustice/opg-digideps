module "database" {
  ...
  preferred_backup_window             = var.account.environment.name == "preproduction" ? "" : "23:00-23:30"
...
locals {
  database = {
    ...
    cluster_resource_id = module.database[0].cluster_resource_id
  }
}
...
resource "aws_route53_record" "api_postgres" {
  ...
  records = [local.database.endpoint]
  ttl     = 300
}
