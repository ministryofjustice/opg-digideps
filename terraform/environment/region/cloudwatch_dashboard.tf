  ...
  dashboard_body = jsonencode({
    "widgets" : [
      ...
      },
      {
        ...
        "properties" : {
          "metrics" : [
            ["AWS/ElastiCache", "CPUUtilization", "CacheClusterId", "${local.front_redis.id}-001", { "label" : "Frontend" }],
            ["AWS/ElastiCache", "CPUUtilization", "CacheClusterId", "${local.api_redis.id}-001", { "label" : "API" }]
          ],
          "view" : "timeSeries",
          "stacked" : false,
          "region" : "eu-west-1",
