resource "aws_iam_role" "document_sync" {
...
resource "aws_iam_role_policy" "document_sync_s3" {
  name   = "document-sync-s3.${local.environment}"
  policy = data.aws_iam_policy_document.front_s3.json
  role   = aws_iam_role.document_sync.id
}

# ======= INVOKE API GATEWAY PERMISSIONS =====
resource "aws_iam_role_policy" "document_sync_invoke_api_gateway" {
  name   = "document-sync-api-gw.${local.environment}"
  policy = data.aws_iam_policy_document.document_sync_invoke_api_gateway.json
  role   = aws_iam_role.document_sync.id
}

data "aws_iam_policy_document" "document_sync_invoke_api_gateway" {
  statement {
...
resource "aws_iam_role_policy" "document_sync_query_ssm" {
  name   = "document-sync-query-ssm.${local.environment}"
  policy = data.aws_iam_policy_document.document_sync_query_ssm.json
  role   = aws_iam_role.document_sync.id
}

data "aws_iam_policy_document" "document_sync_query_ssm" {
  statement {
...
#  policy = data.aws_iam_policy_document.document_sync_query_secretsmanager.json
#  role   = aws_iam_role.document_sync.id
#}
#
#data "aws_iam_policy_document" "document_sync_query_secretsmanager" {
#  statement {
...
#resource "aws_iam_role_policy" "document_sync_get_log_events" {
#  name   = "document-sync-get-log-events.${local.environment}"
#  policy = data.aws_iam_policy_document.document_sync_get_log_events.json
#  role   = aws_iam_role.document_sync.id
#}
#
#data "aws_iam_policy_document" "document_sync_get_log_events" {
#  statement {
