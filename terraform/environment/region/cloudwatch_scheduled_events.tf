  ...
  input = jsonencode(
    {
      "containerOverrides" : [
        {
          "name" : "api_app",
          "command" : ["sh", "scripts/task_run_console_command.sh", "digideps:api:api-comparison", "client/get-all-clients-by-deputy-uid/{deputy_uid}", "v2/deputy/{deputy_id}/courtorders"]
        }
      ]
    }
  )
