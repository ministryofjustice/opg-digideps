...
resource "aws_elasticache_replication_group" "cache_api" {
  ...
  tags = merge({
    InstanceName = "api-${local.account.name}"
    Stack        = local.account.name
  }, local.default_tags)
}
...
resource "aws_security_group" "cache_api_sg" {
  ...
  tags   = merge(local.default_tags, { Name = "${local.account.name}-account-cache--api" })
...
resource "aws_elasticache_replication_group" "front_api" {
  ...
  transit_encryption_enabled = false
  tags = merge({
    InstanceName = "front-${local.account.name}"
    Stack        = local.account.name
  }, local.default_tags)
}
...
resource "aws_security_group" "cache_front_sg" {
  ...
  vpc_id = aws_vpc.main.id
  tags   = merge(local.default_tags, { Name = "${local.account.name}-account-cache-frontend" })
