...
use OPG\Digideps\Frontend\Form\Report\ProfDeputyCostHowType;
use OPG\Digideps\Frontend\Form\Report\ProfDeputyCostInterimType;
use OPG\Digideps\Frontend\Form\Report\ProfDeputyCostPreviousType;
use OPG\Digideps\Frontend\Form\Report\ProfDeputyCostSccoType;
use OPG\Digideps\Frontend\Form\Report\ProfDeputyFixedCostType;
use OPG\Digideps\Frontend\Form\Report\ProfDeputyOtherCostsType;
...
class ProfDeputyCostsController extends AbstractController
{
    ...
    public function previousReceived(Request $request, int $reportId, ?int $previousReceivedId = null): RedirectResponse|array
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($editMode) { // edit
                ...
                $this->addFlash('notice', $this->translator->trans('notices.entry.edited', domain: 'report-prof-deputy-costs'));
            } else {
                $this->restClient->post("/report/$reportId/prof-deputy-previous-cost", $pr, ['profDeputyPrevCosts']);
                $this->addFlash('notice', $this->translator->trans('notices.entry.added', domain: 'report-prof-deputy-costs'));
            }
            ...
            return $this->redirectToRoute($nextRoute, $parameters);
        }
    ...
    public function previousCostDelete(Request $request, int $reportId, int $previousReceivedId): RedirectResponse|array
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('notice', $this->translator->trans('notices.entry.deleted', domain: 'report-prof-deputy-costs'));
