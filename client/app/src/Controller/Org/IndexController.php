...
class IndexController extends AbstractController
{
    ...
    public function dashboardAction(Request $request): RedirectResponse|array
    {
        $currentFilters = [
            'q' => $request->get('q'),
     ...
     */
    #[Route(path: '/client/{clientId}/edit', name: 'org_client_edit')]
    #[Template('@App/Org/Index/clientEdit.html.twig')]
    public function clientEditAction(Request $request, string $clientId): RedirectResponse|array
    {
        try {
            /** @var Client $client */
        ...
        $form = $this->form->create(ClientType::class, clone $client);
     ...
     * Client archive page.
     */
    #[Route(path: '/client/{clientId}/archive', name: 'org_client_archive')]
    #[Template('@App/Org/Index/clientArchive.html.twig')]
    public function clientArchiveAction(Request $request, string $clientId, TranslatorInterface $translator): RedirectResponse|array
    {
        /** @var Client $client */
        $client = $this->restClient->get('client/' . $clientId, 'Client', ['client', 'report-id', 'current-report']);
        ...
        $form = $this->createForm(ClientArchiveType::class, $client);
        $form->handleRequest($request);
