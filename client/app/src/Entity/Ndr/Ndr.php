...
class Ndr implements ReportInterface
{
    ...
    public function validForSubmission()
    {
        $valid = true;
        $msg = [];
        if ($this->nullOrEmptyString($this->getClient()->getCourtDate())) {
            $valid = false;
            $msg[] = 'Missing Court Date on Client';
        }
        if ($this->nullOrEmptyString($this->getClient()->getCaseNumber())) {
            $valid = false;
            $msg[] = 'Missing CaseNumber on Client';
        }
        if ($this->nullOrEmptyString($this->getClient()->getAddress())) {
            $msg[] = 'Missing Address on Client';
        }
        if ($this->nullOrEmptyString($this->getClient()->getPostcode())) {
    ...
    private function nullOrEmptyString($value)
    {
