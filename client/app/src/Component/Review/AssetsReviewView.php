...
final class AssetsReviewView
{
    ...
    private function makeList(Report $report): ListEntries
    {
        ...
        $builder->addEntry($this->l10n['hasAssets'], $this->l10n[match ($report->getNoAssetToAdd()) {
            true => 'no',
            false => 'yes',
            default => 'notEntered'
        }]);
    ...
    private function makeTableOther(string $title, AssetOther ...$assets): Table
    {
        ...
        foreach ($assets as $asset) {
            $builder->addRow(
                $asset->getDescription() ?? $this->l10n['notEntered'],
    ...
    private function makePropertyTables(AssetProperty ...$assets): array
    {
        ...
        foreach ($assets as $key => $asset) {
            $index = $key + 1;
            $builder = new TableBuilder()->addHeader(new Cell("{$this->l10n['property']} {$index}", size: 2), '');

            $builder->addRow($this->l10n['address1'], $asset->getAddress() ?? 'notEntered');
            ...
            $builder->addRow($this->l10n['postcode'], $asset->getPostcode() ?? 'notEntered');
            $builder->addRow($this->l10n['occupants'], $asset->getOccupants() ?? 'notEntered');
            ...
            $builder->addRow($this->l10n['hasMortgage'], $this->l10n[$asset->getHasMortgage() ?? 'notEntered']);
            ...
            $builder->addRow($this->l10n['isSubjectToEquityRelease'], $this->l10n[$asset->getIsSubjectToEquityRelease() ?? 'notEntered']);
            $builder->addRow($this->l10n['hasCharges'], $this->l10n[$asset->getHasCharges() ?? 'notEntered']);
            $builder->addRow($this->l10n['isRentedOut'], $this->l10n[$asset->getIsRentedOut() ?? 'notEntered']);
            if ($asset->getIsRentedOut() === 'yes') {
                $builder->addRow($this->l10n['rentAgreementEndDate'], $asset->getRentAgreementEndDate() === null ? $this->l10n['notEntered'] : $asset->getRentAgreementEndDate()->format('F Y'));
