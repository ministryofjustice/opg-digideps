...
final class DebtsReviewView
{
    ...
    private function makeList(Report $report): ListEntries
    {
        ...
        $hasDebts = $report->getHasDebts() ?: 'blank';

        $builder->addEntry($this->l10n['hasDebts'], $this->translate("review.{$hasDebts}"));
    ...
    private function makeTable2(Report $report): ?Table
    {
        ...
        if ($other !== null && $otherAmount > 0.0) {
            $builder->addRow("{$this->l10n['otherDebt']} {$this->formatMoney($otherAmount)}", $other->getMoreDetails() ?? '');
        }
        $builder->addRow($this->l10n['howManaged'], $report->getDebtManagement() ?? '');
