...
def run_get(lambda_client, query_id):
    if not query_id:
...
def run_sign_off(lambda_client, query_id, calling_user):
    if not query_id:
        print("Supply the query_id argument")
        sys.exit(1)
...
def run_revoke(lambda_client, query_id):
    if not query_id:
        print("Supply the query_id argument")
        sys.exit(1)
...
def run_execute(lambda_client, query_id, calling_user):
    if not query_id:
        print("Supply the query_id argument")
        sys.exit(1)
...
def main(
    ...
    if action == "insert":
        ...
        response = run_get(lambda_client, query_id)
    elif action == "sign_off":
        response = run_sign_off(lambda_client, query_id, calling_user)
    elif action == "revoke":
        response = run_revoke(lambda_client, query_id)
    elif action == "execute":
        response = run_execute(lambda_client, query_id, calling_user)
    else:
        print("Not a valid action")
        sys.exit(1)

    print(json.dumps(response["body"]["result"], indent=4))
