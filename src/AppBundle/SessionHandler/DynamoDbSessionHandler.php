...
class DynamoDbSessionHandler implements SessionHandlerInterface {
    ...
    {
        $client = DynamoDbClient::factory($config['client']);
        $config['session']['dynamodb_client'] = $client;
        $this->handler = \Aws\DynamoDb\Session\SessionHandler::factory($config['session']);
    }
