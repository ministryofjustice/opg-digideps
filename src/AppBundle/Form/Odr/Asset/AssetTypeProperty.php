    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        if ($this->step === 3) {
            $builder->add('owned', 'choice', array(
                ...
                ->add('ownedPercentage', 'number', [
                    ...
                    'max_length' => 2,
                    'pattern' => '[0-9]',
                    'invalid_message' => 'odr.asset.property.ownedPercentage.type',
                ]);
        }
        if ($this->step === 4) {
            $builder->add('hasMortgage', 'choice', [
                ...
                ->add('mortgageOutstandingAmount', 'number', [
                    'grouping' => true,
                    'precision' => 2,
                    'invalid_message' => 'odr.asset.property.mortgageOutstandingAmount.type',
                ]);
        }
        ...
        if ($this->step === 5) {
            $builder->add('value', 'number', [
                'grouping' => true,
                'precision' => 2,
                'invalid_message' => 'odr.asset.property.value.type',
            ]);
        }
        ...
        if ($this->step === 8) {
            $builder
                ...
                ->add('rentIncomeMonth', 'number', [
                    'grouping' => true,
                    'precision' => 2,
                    'invalid_message' => 'odr.asset.property.rentIncomeMonth.type',
                ]);
