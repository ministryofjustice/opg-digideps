...
case "$INTEGRATION_SELECTION" in
  ...
  selection-3)
    ...
    php vendor/bin/phpunit -c tests/Integration tests/Integration/Service/ --coverage-php tests/coverage/Controller.cov
    ;;
  selection-all)
    # selection-1
    printf '\n Running Controller Suite \n\n'
  ...
  selection-solo)
    ...
    php vendor/bin/phpunit -c tests/Integration $TEST_FILTER "tests/Integration/$SUITE" --coverage-php tests/coverage/integration-solo.cov
