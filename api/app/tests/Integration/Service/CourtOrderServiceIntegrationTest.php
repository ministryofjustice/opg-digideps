...
class CourtOrderServiceIntegrationTest extends ApiIntegrationTestCase
{
    ...
    public function testGetCourtOrderSingleReport(): void
    {
        ...
        // --- Arrange ---
        // User who will be authorised (should implement UserInterface in your project)
        $user = (UserTestHelper::create())->createAndPersistUser($em, email: $email);

        // Deputy tied to the user (helpers usually link this correctly)
        $deputy = DeputyTestHelper::generateDeputy($email, $deputyUid, $user);
        $em->persist($deputy);
        // Client + Court Order + Report using helpers
        ...
        $em->persist($report);
        // If your helpers allow passing the uid, use it directly; otherwise set it on the court order.
