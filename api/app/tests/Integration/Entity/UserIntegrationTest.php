...
class UserIntegrationTest extends ApiIntegrationTestCase
{
    public function testGetNumberOfSubmittedReports()
    {
        ...
        ['orders' => [['pfa' => ['reports' => [$report]]]]] = self::$fixtureService->instantiateScenario(new Scenario(new CourtOrderDescriptor(DeputySet::oneLay(), submittedReports: 2)));
        $submittedSubmissions1 = $report->getReportSubmissions()->first() ?: throw new \LogicException("This must exist");

        ['orders' => [['pfa' => ['reports' => [$report]]]]] = self::$fixtureService->instantiateScenario(new Scenario(new CourtOrderDescriptor(DeputySet::oneLay(), submittedReports: 1)));
