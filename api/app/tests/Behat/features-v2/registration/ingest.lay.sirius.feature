@v2 @v2_sequential_4 @registration @ingest
...
#    ToDo Fix this test as it is breaking in our pipeline
    @super-admin @broken-e2e-ingest-test
    Scenario: Uploading a Lay CSV that contains details of a new deputyship for an existing Lay deputy with a single active client
        ...
        And the Lay deputy with deputy UID 700762222001 has 1 associated active clients
        Then a csv has been uploaded to the sirius bucket with the file 'lay-2-rows-deputy-has-multiple-client-deputyships.csv'
        When I run the lay CSV command the file contains 2 new pre-registration entities
        And the Lay deputy with deputy UID 700762222001 has 2 associated active clients
