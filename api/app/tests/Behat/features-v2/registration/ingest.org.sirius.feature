...
Feature: Org CSV data ingestion - sirius source data
    ...
    Scenario: Uploading a CSV that contains existing clients and named deputies - new named deputy in same firm
        Given a super admin user accesses the admin app
        When I upload an org CSV that has a new named deputy 'MAYOR MCCRACKEN' within the same org as the clients existing name deputy
    ...
    Scenario: Uploading a CSV that contains existing clients and named deputies - named deputy address and phone updated
        Given a super admin user accesses the admin app
        When I upload an org CSV that has a new address '75 Plutonium Way, Salem, Witchington, Barberaham, Townsville, TW5 V78' for an existing named deputy
    ...
    Scenario: Uploading a CSV that contains existing clients and named deputies - report type updated
        Given a super admin user accesses the admin app
        Then I upload an org CSV that has a new report type '103-5' for an existing report that has not been submitted or unsubmitted
    ...
    Scenario: Uploading a CSV that contains an existing dual case - report type updated only when deputy uid matches
        Given a super admin user accesses the admin app
        When I upload an org CSV that has a new report type '103-5' for a dual case
...
#        And the count of the new 'org' entities added should be displayed on the page
    ...
    Scenario: Uploading a CSV that has an organisation name but missing deputy first and last name
        Given a super admin user accesses the admin app
        When I upload an org CSV that has an organisation name 'Conglom-O Corporation' but missing deputy first and last name
    ...
    Scenario: Uploading a CSV that contains deputy name updates for existing deputies
        Given a super admin user accesses the admin app
        When I upload an org CSV that has one person deputy and one organisation deputy
        Then the new 'org' entities should be added to the database
        And the count of the new 'org' entities added should be displayed on the page
        When I upload an org CSV that updates the person deputy with an org name and the org deputy with a person name
    ...
    Scenario: Uploading a CSV that contains deputy email updates for existing deputies
        Given a super admin user accesses the admin app
        When I upload an org CSV that has one person deputy and one organisation deputy 2nd run
        Then the new 'org' entities should be added to the database
        And the count of the new 'org' entities added should be displayed on the page
