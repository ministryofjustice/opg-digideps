    ...
    public function __construct(
        ...
        foreach ($rows as $row) {
            ...
            if (!array_key_exists($orderId, $orders)) {
                $orders[$orderId] = new Order($this, $orderId, $row['order_uid'], new \DateTimeImmutable($row['order_made_date']), (bool)$row['open']);
            }
            $reports[$reportId] ??= [
                'orders' => [],
                'report_id' => $reportId,
                'start_date' => new \DateTimeImmutable($row['report_start_date']),
                'end_date' => new \DateTimeImmutable($row['report_end_date']),
