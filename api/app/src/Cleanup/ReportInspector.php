...
final readonly class ReportInspector
{
    ...
    public function hasMadeDateBeforeStartOrEndDate(): bool
    {
        ...
        foreach ($this->report->orders as $order) {
            if ($order->madeDate->getTimestamp() < $madeDate->getTimestamp()) {
                $madeDate = $order->madeDate;
            }
        }
        return $madeDate->getTimestamp() <= $this->report->endDate->getTimestamp() || $madeDate->getTimestamp() <= $this->report->startDate->getTimestamp();
    }
    ...
    public function hasNoActiveOrders(): bool
    {
        return empty(array_filter($this->report->orders, fn (Order $order) => $order->open));
    }
    ...
    public function getCleaningActions(): array
    {
        ...
        if (empty($actions)) {
            $keptOrder = null;
            foreach (array_filter($this->report->orders, fn (Order $order) => $order->madeDate->getTimestamp() <= $this->report->startDate->getTimestamp()) as $order) {
                $keptOrder ??= $order;
                if ($order->madeDate->getTimestamp() > $keptOrder->madeDate->getTimestamp()) {
                    $keptOrder = $order;
                }
            }
