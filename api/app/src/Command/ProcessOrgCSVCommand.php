    ...
    private function process(mixed $data): bool
    {
        if (is_array($data)) {
            ...
            foreach ($chunks as $index => $chunk) {
                ...
                $memoryUsageMegabytes = $mu / (1024 * 1024);
                $formattedMemoryUsage = number_format($memoryUsageMegabytes, 2);
                $this->verboseLogger->warning('memory after orgProcessing: '.$formattedMemoryUsage.'mb - '.$index);
                ...
                $memoryUsageMegabytes = $mu / (1024 * 1024);
                $formattedMemoryUsage = number_format($memoryUsageMegabytes, 2);
                $this->verboseLogger->warning('memory after storeOutput: '.$formattedMemoryUsage.'mb - '.$index);
            }
