...
class ReportController extends RestController
{
    ...
    public function submit(Request $request, int $id): ?int
    {
        ...
        $explanation = ($data['agreed_behalf_deputy'] === 'more_deputies_not_behalf')
        ...
        if (array_key_exists('prof_deputy_other_costs', $data)) {
            ...
            foreach ($data['prof_deputy_other_costs'] as $postedProfDeputyOtherCostType) {
                if (
                    ...
                    if ($profDeputyOtherCost instanceof ProfDeputyOtherCost) {
                        $profDeputyOtherCost->setAmount($postedProfDeputyOtherCostType['amount']);
                    } else {
                        $profDeputyOtherCost = new ProfDeputyOtherCost(
                            $report,
                            $postedProfDeputyOtherCostType['prof_deputy_other_cost_type_id'],
    ...
    private function getResponseByUser(Request $request): array
    {
        ...
        $data = $this->repository->getAllByUser($user->getId(), $request->query, $request->query->getString('status'));
    ...
    private function getCountOfReportsByStatus(string $status, int $userId, Request $request): int
    {
        return $this
            ->repository
            ->getAllByUserCount($userId, $request->query, $status);
    }
    ...
    public function insertChecklist(Request $request, int $report_id): array
    {
        ...
        if (!empty($checklistData['further_information_received'])) {
            $info = new ChecklistInformation($checklist, $checklistData['further_information_received']);
    ...
    public function getChecklist(int $report_id)
    {
        ...
        return $this->em
            ->getRepository(ReviewChecklist::class)
    ...
    public function upsertChecklist(Request $request, int $report_id)
    {
        ...
        /** @var ReviewChecklist|null $checklist */
        $checklist = $this->em
            ->getRepository(ReviewChecklist::class)
            ->findOneBy(['report' => $report->getId()]);

        if (is_null($checklist)) {
            $checklist = new ReviewChecklist($report);
        }
