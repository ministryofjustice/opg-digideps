...
class ReportRepository extends ServiceEntityRepository
{
    ...
    private function getAllByUserQuery(int $userId, ParameterBag $query, string $select, string $status): ?QueryBuilder
    {
        $qb = $this->createQueryBuilder('r')
    ...
    public function getAllByUserCount(int $userId, ParameterBag $query, string $status): int
    {
        $qb = $this->getAllByUserQuery($userId, $query, 'count', $status);
        $count = $qb?->getQuery()->getSingleScalarResult() ?? 0;
        return (int)$count;
    }

    public function getAllByUser(int $userId, ParameterBag $query, string $status): array
    {
        return $this->getAllByUserQuery($userId, $query, 'reports', $status)
