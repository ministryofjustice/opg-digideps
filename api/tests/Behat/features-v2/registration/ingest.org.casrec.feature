...
Feature: Org CSV data ingestion - casrec source data
    ...
    Scenario: Uploading a CSV that contains new clients and named deputies only
        ...
        And the count of the new entities added should be displayed on the page
    ...
    Scenario: Uploading a CSV that contains existing clients and named deputies - client made date and new named deputy in same firm
        ...
        And I upload a 'casrec' CSV that has a new made date '20-Aug-2019' and named deputy 'MAYOR MCCRACKEN' within the same org as the clients existing name deputy
    ...
    Scenario: Uploading a CSV that contains existing clients and named deputies - named deputy address and phone updated
        Given a super admin user accesses the admin app
        When I visit the admin upload org users page
        And I upload a 'casrec' CSV that has a new address '75 Plutonium Way, Salem, Witchington, Barberaham, Townsville, TW5 V78' for an existing named deputy
    ...
    Scenario: Uploading a CSV that contains deputies with missing required information alongside valid deputy rows
        Given a super admin user accesses the admin app
        When I visit the admin upload org users page
        And I upload a 'casrec' CSV that has 1 row with missing values 'Last Report Day, Made Date, Email' for case number '70000000' and 1 valid row
        Then I should see an error showing the problem
        And the new entities should be added to the database
        And the count of the new entities added should be displayed on the page
    ...
    Scenario: Uploading a CSV that has missing required columns
        Given a super admin user accesses the admin app
        When I visit the admin upload org users page
        And I upload a 'casrec' CSV that does not have any of the required columns
    ...
    Scenario: Uploading a CSV that has an unexpected column
        Given a super admin user accesses the admin app
        When I visit the admin upload org users page
        And I upload a 'casrec' CSV that has an 'NDR' column
