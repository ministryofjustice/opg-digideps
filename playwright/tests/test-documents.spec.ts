...
test("a user has no supporting documents to add", async ({ page }) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    ...
    await reportOverviewPage.expectSectionStatus(
...
test("a user uploads multiple supporting documents with valid file types", async ({
  page,
}) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    const reportId = scenario.orders[0].reports[1].id;
...
test("a user uploads a file with an invalid file type", async ({ page }) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    ...
    const documentsUploadPage = new DocumentsUploadPage(page, reportId);
    await documentsUploadPage.attachFiles(
      path.join(__dirname, "/testFiles/badfiletype.txt"),
    );
...
test("a user uploads a file whose suffix does not match its type", async ({
  page,
}) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    const reportId = scenario.orders[0].reports[1].id;
    ...
    const documentsUploadPage = new DocumentsUploadPage(page, reportId);
    await documentsUploadPage.attachFiles(
      path.join(__dirname, "/testFiles/pngfile.jpeg"),
    );

    const errors = new PageErrorMessage(page);
    await errors.expectErrorMessage(
      "Your file type and file extension do not match",
    );
  };
...
test("a user uploads a file whose name duplicates an existing upload", async ({
  page,
}) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    const reportId = scenario.orders[0].reports[1].id;
...
test("a user uploads a file which is too large", async ({ page }) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    ...
    const errors = new PageErrorMessage(page);
    await errors.expectErrorMessage(
      "The file you selected to upload is too big",
    );
  };
...
test("a user uploads multiple supporting documents with valid file types which require conversion", async ({
  page,
}) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    const reportId = scenario.orders[0].reports[1].id;
...
test("a user deletes documents", async ({ page }) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    ...
    await banner.expectSuccessMessage(
      "File named testimage1.png has been removed",
    );
    await documentsUploadPage.expectFileNames(
      "testimage2.png",
      "testimage3.png",
    );
    ...
    await banner.expectSuccessMessage(
      "File named testimage2.png has been removed",
    );
...
test("a user uploads a file and then selects 'no' for supporting documents", async ({
  page,
}) => {
  const runTest = async (scenario: Scenario): Promise<void> => {
    const reportId = scenario.orders[0].reports[1].id;
    ...
    const documentsFrontPage = await startDocumentsSection(
      page,
      email,
      reportId,
      "yes",
    ...
    await errors.expectErrorMessage(
      "Your answer could not be updated to 'No' because you have attached documents",
    );
  };
