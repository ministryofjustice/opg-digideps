...
export default class AdminDocumentsListPage {
  ...
  async expectDocumentsWithStatuses(
    ...expectedFileNamesAndStatuses: Array<{ name: RegExp; status: string }>
  ): Promise<void> {
    ...
    for (const documentSubmission of documentSubmissions) {
      let fileName = await documentSubmission
        .locator("td")
        .nth(0)
        .textContent();
      ...
      if (fileName !== null && status !== null) {
        ...
        } of expectedFileNamesAndStatuses) {
    ...
    expect(documentSubmissions.length).toBe(
      expectedFileNamesAndStatuses.length,
    );
  }
}
