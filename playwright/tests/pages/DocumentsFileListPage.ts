...
export default abstract class DocumentsFileListPage {
  private fileNamesSelector: string =
    "dl.behat-region-document-list > div.govuk-summary-list__row > dt.govuk-summary-list__value";
  ...
  async removeFiles(...fileNames: string[]) {
    for (const fileName of fileNames) {
      await this.page
        .getByRole("link", { name: `Remove  ${fileName}` })
        .click();
    }
  }

  async expectFileNames(...expectedFileNames: string[]) {
    const fileNameElements = await this.page
      .locator(this.fileNamesSelector)
      .all();
