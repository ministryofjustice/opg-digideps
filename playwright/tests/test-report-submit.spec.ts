...
test("if a document's S3 object is gone, user is redirected to re-upload it on submitting report", async ({
  page,
}) => {
  const runTest = async (scenario: Scenario) => {
    ...
    const reportDocumentsReuploadPage = new ReportDocumentsReuploadPage(
      page,
      reportId,
    );
    ...
    await reportDocumentsReuploadPage.uploadFiles(
      path.join(__dirname, `/testFiles/${supportingDocument}`),
    );
