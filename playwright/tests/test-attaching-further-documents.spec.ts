...
test("a user sends further documents", async ({ page }) => {
  ...
  const runTest = async (scenario: Scenario) => {
    ...
    for (const fileToUpload of filesToUpload) {
      ...
      const fileToUploadFullPath = path.join(
        __dirname,
        `/testFiles/${fileToUpload}`,
      );
    ...
    await expect(
      page.locator(".behat-region-report-submission-documents-1"),
    ).toContainText(filesToUpload[0]);
    await expect(
      page.locator(".behat-region-report-submission-documents-2"),
