...
locals {
  integration_test_container = <<EOF
  {
    ...
    "environment": [
      { "name": "PGHOST", "value": "${aws_rds_cluster.api.endpoint}" },
      { "name": "PGDATABASE", "value": "${aws_rds_cluster.api.database_name}" },
      { "name": "PGUSER", "value": "${aws_rds_cluster.api.master_username}" },
