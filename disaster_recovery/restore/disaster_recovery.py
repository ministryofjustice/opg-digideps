...
class SnapshotManagement:
    def __init__(
        ...
        self.account = None if environment not in environments.keys() else environments[environment]
        ...
        self.backup_role_to_assume = str(f'arn:aws:iam::{self.backup_account}:role/{self.role}')
        ...
        self.db_cluster_identifier_target = self.db_cluster_identifier_source if cluster_to is None else cluster_to
        self.same_target = True if self.db_cluster_identifier_target == self.db_cluster_identifier_source else False
    ...
    def restore(self):
        ...
        if self.db_cluster_identifier_source is not None:
            ...
            if self.db_cluster_identifier_source == self.db_cluster_identifier_target and self.DeletionProtection:
                self.drop_protection()

        self.apply_overrides()

        no_point_in_time_argument_set = datetime.strptime('1900-01-01 00:00:00', '%Y-%m-%d %H:%M:%S')
        ...
        if self.PointInTimeRecovery == no_point_in_time_argument_set:
            if self.SnapshotIdentifier is not None:
                ...
                print('No snapshot specified. Either specify snapshot or do point in time recovery')
        else:
    ...
    def get_instance_info(self):
        response = self.client.describe_db_instances(
            Filters=[{'Name': 'db-cluster-id', 'Values': [self.db_cluster_identifier_source]}],
        )
    ...
    def get_cluster_info(self):
        ...
        self.db_cluster_identifier_source = response['DBClusters'][0]['DBClusterIdentifier']
        self.DBClusterParameterGroup = response['DBClusters'][0]['DBClusterParameterGroup']
        ...
        self.Capacity = None if 'Capacity' not in response['DBClusters'][0] else response['DBClusters'][0]['Capacity']
        ...
        self.AutoMinorVersionUpgrade = response['DBClusters'][0]['AutoMinorVersionUpgrade']
        self.HttpEndpointEnabled = response['DBClusters'][0]["HttpEndpointEnabled"]

    def restore_from_snapshot(self):
        if self.same_target:
            self.db_cluster_identifier_target = f"{self.db_cluster_identifier_source}-temp"
    ...
    def overwrite_existing_cluster(self):
        ...
        self.rename_cluster(
            self.db_cluster_identifier_source,
            source_backup
        )
    ...
    def get_instance_id(self, instance, temp=True):
        return f'{instance["DBInstanceIdentifier"]}-temp' if self.same_target and temp else instance["DBInstanceIdentifier"]
    ...
    def apply_overrides(self):
        self.MultiAZ = self.MultiAZ if self.multi_az_override is None else self.multi_az_override
    ...
    def wait_on_cluster_available(self, cluster=None):
        cluster_to_await = self.db_cluster_identifier_target if cluster is None else cluster
        print(f'Starting to wait on cluster {cluster_to_await}')
        total_time = 0
        sleep_time = 30
        while True:
            response = self.client.describe_db_clusters()
            cluster_from_response = [cluster for cluster in response["DBClusters"] if cluster["DBClusterIdentifier"] == cluster_to_await]

            if len(cluster_from_response) > 0:
                status = cluster_from_response[0]["Status"]
                if status == 'available':
                    print(f'Cluster {cluster_to_await} exists but is not available yet. Status: {status}')
                    break
            else:
                print(f"Cluster {cluster_to_await} does not exist yet")
            time.sleep(sleep_time)
            total_time += sleep_time
            print(f'Waiting for cluster {cluster_to_await} to become available for {total_time} seconds')
    ...
    def wait_on_cluster_deleted(self, cluster=None):
        ...
        while True:
            ...
            if len(cluster_from_response) > 0:
                ...
                print(f"Cluster {cluster_to_await} no longer exists. Deletion successful")
                break
            time.sleep(sleep_time)
            total_time += sleep_time
            print(f'Waiting for cluster {cluster_to_await} to be deleted for {total_time} seconds')
    ...
    def wait_on_instance_available(self, temp_instances=True):
        ...
        for instance in self.instances:
            ...
            while True:
                response = self.client.describe_db_instances()
                instance_from_response = [response_instance for response_instance in response["DBInstances"] if
                                          response_instance["DBInstanceIdentifier"] == instance_id]

                if len(instance_from_response) > 0:
                    status = instance_from_response[0]["DBInstanceStatus"]
                    if status == 'available':
                        print(f'Instance {instance_id} exists but is not available yet. Status: {status}')
                        break
                else:
                ...
                time.sleep(sleep_time)
                total_time += sleep_time
                print(f'Waiting for instance {instance_id} to become available for {total_time} seconds')
        ...
        for instance in self.instances:
            ...
            while True:
                ...
                if len(instance_from_response) > 0:
                    ...
                    print(f"Cluster {instance_id} no longer exists. Deletion successful")
                    break
                time.sleep(sleep_time)
                total_time += sleep_time
                print(f'Waiting for instance {instance_id} to be deleted for {total_time} seconds')

    def share_snapshot_with_digideps(self):
        print(f"Sharing snapshot {self.SnapshotIdentifier} with account {self.account}...")
    ...
    def share_snapshot_with_backup(self):
        print(f"Sharing snapshot {self.SnapshotIdentifier} with account {self.backup_account}...")
        ...
        print(f"Snapshot {self.SnapshotIdentifier} shared with account {self.backup_account}")
    ...
    def create_digideps_client_session(self):
        ...
        self.client_kms = autorefresh_session.client(
    ...
    def command_response(command, response):
        print(f'{command} command responded with status: {response["ResponseMetadata"]["HTTPStatusCode"]}')
...
@click.command()
@click.option(
    "--environment", default="development", help="Environment to restore into (development/preproduction/production)"
)
@click.option(
    "--cluster_from", default=None, help="Cluster for getting the settings from and for point in time restores"
)
@click.option(
    "--cluster_to", default=None, help="Cluster to restore into. Leave blank if same as cluster_from"
)
@click.option(
    "--snapshot_id", default=None, help="Snapshot to use for the restore (not needed for point in time recovery)"
)
@click.option(
    "--pitr", default='1900-01-01 00:00:00', help="Date for Point in Time Recovery (default means don't use pitr)"
)
@click.option(
    "--multi_az_override", default=None, help="Make restored cluster a different az spec than original"
)
@click.option(
    "--engine_mode_override", default=None, help="Make restored cluster a different engine mode than original"
)
@click.option(
    "--restore_from_remote", default=False, help="Whether to restore from remote backup"
)
def main(
    environment, cluster_from, cluster_to, snapshot_id, multi_az_override, engine_mode_override, pitr, restore_from_remote
):
